...
<table width="600" align="center" style="color:white">
	<tr>
	<th>Game name</th>
	<th>Game cost</th>
	<tr>
	<c:forEach items="${sessionScope.cbo.getCboList()}" var="c">
		<tr>
			<form method="post" action="removeFromCart?gid=${c.gid}">
			<td>${c.gname}</td>
			<td>${c.finalcost} VND</td>
			<td><button type="submit" class="btn" style="background-color:red"><i class="fa fa-close"></i></button></td>
			</form>
		</tr>
	</c:forEach>
</table>
<div style="text-align:right"><text style="color:#9DB9BF">Total Cost : </text><text>${sessionScope.cbo.totalcost()} VND</text></div><br>
<form method="post" action="purchase?un=${sessionScope.u.un}">
<p style="text-align:right"><input type="submit" class="btn btn-danger" value="Purchase" name="buysubmit"></p>
</form>
